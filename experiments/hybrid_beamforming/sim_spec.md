...
### **2.4. シミュレーションの前提と制約 (Assumptions & Limitations)**

本シミュレーションは、以下の前提に基づいて実装されている：

1.  **SU-MIMO / 干渉許容 MU-MIMO**:
    *   デジタルプリコーディングはユーザごとの SVD (Singular Value Decomposition) に基づいており、他ユーザへの干渉をゼロにする Zero-Forcing (ZF) は適用されない。
    *   したがって、同一リソース（時間・周波数）に複数ユーザを多重化（MU-MIMO）する場合、**ユーザ間干渉 (Inter-User Interference) が発生する** 仕様である。
2.  **全帯域割り当て**:
    *   OFDMAのような周波数リソース割り当て（スケジューリング）は行わず、全ユーザが全帯域幅を使用すると仮定する。
3.  **アナログビームのパネル割り当て**:
    *   BSの各アンテナパネルは、**Round-robin** 方式でユーザに割り当てられる（Panel $j$ $\rightarrow$ User $(j \pmod{N_{user}})$）。
    *   SU-MIMO ($N_{user}=1$) の場合、全パネルがそのユーザに向けられ、アレイゲインが最大化される。
