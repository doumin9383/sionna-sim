...
`path_gains` の複素振幅は、**送信電力 1W (30dBm)** で正規化してください．
...
#### 送受信の役割スワップ
データをSionna (Downlink基準) に合わせるため、データ変換時に **BS側の角度情報をTx、UE側の角度情報をRx** としてマッピングし直します。

#### 角度体系の変換
Volcanoなどの角度系（北=0, 水平=0）から、Sionnaの角度系（東=0, 天頂=0）への変換は、データ生成時（HDF5作成時）に行います
