...
class ResourceGridConfig:
    ...
    pilot_ofdm_symbol_indices: Optional[List[int]] = field(
        default_factory=lambda: [2, 11]
    )


@dataclass
class TBConfigConfig:
    """Configuration for sionna.phy.nr.TBConfig"""

    mcs_index: int = 1
    target_coderate: float = 0.5
    # Add other params like mcs_table_id if needed
